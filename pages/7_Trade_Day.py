...
st.set_page_config(page_title="Trade Day",
...
def draft_day():
    ...
    if is_first_day_of_quarter_today:
        ...
        st.write("Trade Days are on the first day of the calendar quarter. "
                 "Come back soon.")
        return False
    ...
    with st.form("Trade Day"):
        ...
        if submitted:
            ...
            if MATCH:
                ...
                # Execute the query with parameters
                conn.cursor().execute(
                    DELETE_QUERY, (sel_pick,
                                   email,
                                   DRAFT_YEAR)
                )

                sms_message = (user_name + " has traded "
                               + sel_pick
                               + " for "
                               + pick
                               )

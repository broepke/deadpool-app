...
from dp_utilities import check_password, load_snowflake_table, snowflake_connection_helper
import pandas as pd
...
def load_data():
    ...
    df_dead = df.loc[df["DEATH_DATE"].notnull() & (df['YEAR'] == 2024)].sort_values(by="DEATH_DATE", ascending=False)
    return df_dead
...
if authenticated:
    ...
    if search_term:
        df_dead = df_dead[df_dead['NAME'].str.contains(search_term, case=False, na=False)]
    ...
    st.write(f"Showing {start_idx + 1}-{min(end_idx, len(df_dead))} of {len(df_dead)} results")